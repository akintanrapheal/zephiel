...
public class SmtpEmailService : IEmailService
{
    ...
    public async Task<bool> SendAsync(string toEmail, string subject, string innerHtml, string? toName = null, CancellationToken ct = default)
    {
        ...
        if (!smtpConfigured)
        {
            ...
            {
                _log.LogWarning("Email NOT sent (SMTP not configured). to={To} subject=\"{Subject}\"", Zephiel.Web.Infrastructure.LogRedact.Email(toEmail), subject);
        ...
        try
        {
            ...
            _log.LogInformation("Email sent to {To}: \"{Subject}\"", Zephiel.Web.Infrastructure.LogRedact.Email(toEmail), subject);
            await LogEmailAsync(toEmail, toName, subject, true, null, "smtp");
            return true;
        }
        catch (Exception ex)
        {
            _log.LogError(ex, "Failed to send email to {To}: \"{Subject}\"", Zephiel.Web.Infrastructure.LogRedact.Email(toEmail), subject);
    ...
    private async Task<bool> WriteToPickupAsync(string pickupDir, string toEmail, string? toName, string subject, string innerHtml, Branding brand)
    {
        try
        {
            ...
            var safeTo = string.Concat(Zephiel.Web.Infrastructure.LogRedact.Email(toEmail).Split(Path.GetInvalidFileNameChars()));
            ...
            _log.LogInformation("Email written to pickup folder (no SMTP): {File} — to={To} subject=\"{Subject}\"", file, Zephiel.Web.Infrastructure.LogRedact.Email(toEmail), subject);
            return true;
        }
        catch (Exception ex)
        {
            _log.LogError(ex, "Failed to write email to pickup folder. to={To} subject=\"{Subject}\"", Zephiel.Web.Infrastructure.LogRedact.Email(toEmail), subject);
            return false;
        }
    }
