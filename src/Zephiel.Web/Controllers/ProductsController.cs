...
public class ProductsController : Controller
{
    ...
    public async Task<IActionResult> Index(ProductFilterViewModel filters, int page = 1, int pageSize = 60)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filters.Category))
        {
            ...
            var catSlugs = Zephiel.Web.Infrastructure.StoreMenu.ExpandSlug(filters.Category);
    ...
    public async Task<IActionResult> Detail(string slug)
    {
        ...
        Zephiel.Web.Infrastructure.RecentlyViewed.Record(Request, Response, product.Id);
    ...
    public async Task<IActionResult> NotifyRestock(int productId, string email)
    {
            ...
            && await _db.Products.AnyAsync(p => p.Id == productId && p.IsActive))
        {
            ...
            if (!alreadyPending)
            {
                ...
                _logger.LogInformation("Back-in-stock request saved: product {ProductId} for {Email}", productId, Zephiel.Web.Infrastructure.LogRedact.Email(email));
            }
        }
