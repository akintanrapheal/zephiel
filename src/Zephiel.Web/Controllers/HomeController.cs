...
public class HomeController : Controller
{
    ...
    private readonly Zephiel.Web.Services.IMerchandisingService _merch;
    private readonly Zephiel.Web.Services.ISettingsService _settings;
    private readonly Zephiel.Web.Services.Marketing.IMarketingService _marketing;

    public HomeController(ILogger<HomeController> logger, ApplicationDbContext db,
        Zephiel.Web.Services.IMerchandisingService merch,
        Zephiel.Web.Services.ISettingsService settings,
        Zephiel.Web.Services.Marketing.IMarketingService marketing)
    {
        _logger = logger;
        _db = db;
        _merch = merch;
    ...
    public async Task<IActionResult> WelcomeOffer(string email)
    {
        ...
        if (enabled && pct > 0 && isNew)
            code = await _marketing.MintCouponAsync(Zephiel.Web.Models.Domain.DiscountType.Percentage,
    ...
    private static bool IsPosPath(string path)
    {
        ...
        return seg.Equals(Zephiel.Web.Infrastructure.StaffPaths.Pos, StringComparison.OrdinalIgnoreCase)
            || seg.Equals("Till", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsStaffPath(string path) =>
        Zephiel.Web.Infrastructure.StaffPaths.IsStaffPath(path);
}
