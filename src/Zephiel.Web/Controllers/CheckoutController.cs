...
using Zephiel.Web.Models.ViewModels;
...
public class CheckoutController : Controller
{
    ...
    private readonly Zephiel.Web.Services.IOrderFulfilmentService _fulfilment;
    private readonly Zephiel.Web.Services.ISettingsService _settings;
    private readonly Zephiel.Web.Services.DeliveryZoneService _zones;
    private readonly Zephiel.Web.Services.IDiscountService _discounts;
    private readonly Zephiel.Web.Services.IEmailService _email;
    private readonly Zephiel.Web.Services.IWhatsAppService _whatsapp;
    private readonly Zephiel.Web.Services.ILoyaltyService _loyalty;
    private readonly Zephiel.Web.Services.IGiftCardService _giftCards;
    private readonly Zephiel.Web.Services.Logistics.ILogisticsDispatchService _logistics;
    private readonly Zephiel.Web.Services.IStockService _stock;
    private readonly Zephiel.Web.Services.IAuditService _audit;
    private readonly Zephiel.Web.Services.IOrderNumberService _orderNumbers;
    ...
    public CheckoutController(
        ...
        Zephiel.Web.Services.IOrderFulfilmentService fulfilment,
        Zephiel.Web.Services.ISettingsService settings,
        Zephiel.Web.Services.DeliveryZoneService zones,
        Zephiel.Web.Services.IDiscountService discounts,
        Zephiel.Web.Services.IEmailService email,
        Zephiel.Web.Services.IWhatsAppService whatsapp,
        Zephiel.Web.Services.ILoyaltyService loyalty,
        Zephiel.Web.Services.IGiftCardService giftCards,
        Zephiel.Web.Services.Logistics.ILogisticsDispatchService logistics,
        Zephiel.Web.Services.IStockService stock,
        Zephiel.Web.Services.IAuditService audit,
        Zephiel.Web.Services.IOrderNumberService orderNumbers,
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var vm = new CheckoutViewModel
        {
            ...
            NigerianStates = Zephiel.Web.Services.DeliveryZoneService.NigerianStates,
            LagosLGAs = Zephiel.Web.Services.DeliveryZoneService.LagosLGAs,
        ...
        CartViewModel cart, FulfillmentChoice fulfilment, string? state, string? city, int? pickupStoreId)
    {
        ...
        {
            var ranked = Zephiel.Web.Services.DeliveryZoneService.RankStoresByProximity(activeStores, state, city);
        ...
        foreach (var grp in cart.Items.GroupBy(i => (i.ProductId, i.VariantId)))
        {
            ...
            {
                var zone = Zephiel.Web.Services.DeliveryZoneService.GetZone(state ?? "");
                ...
                if (zone == Zephiel.Web.Services.DeliveryZone.National) continue;
                var nearAvail = 0;
                foreach (var s in activeStores.Where(s => Zephiel.Web.Services.DeliveryZoneService.GetZone(s.State) == zone))
    ...
    private async Task RehydrateCheckoutDisplayAsync(CheckoutViewModel vm)
    {
        ...
        vm.NigerianStates      = Zephiel.Web.Services.DeliveryZoneService.NigerianStates;
        vm.LagosLGAs           = Zephiel.Web.Services.DeliveryZoneService.LagosLGAs;
    ...
    public async Task<IActionResult> PlaceOrder(CheckoutViewModel vm)
    {
        ...
        if (user == null)
        {
            ...
            if (user == null)
            {
                ...
                _logger.LogInformation("Guest account created for checkout: {Email}", Zephiel.Web.Infrastructure.LogRedact.Email(vm.GuestEmail));
            }
        }
        ...
        var order = new Order
        {
            ...
            DeviceType = Zephiel.Web.Infrastructure.OrderAttributionMiddleware.DeviceFromUserAgent(Request.Headers.UserAgent.ToString()),
            Origin = HttpContext.Session.GetString(Zephiel.Web.Infrastructure.OrderAttributionMiddleware.OriginKey) ?? "Direct",
            SessionPageViews = HttpContext.Session.GetInt32(Zephiel.Web.Infrastructure.OrderAttributionMiddleware.PageViewsKey),
            Subtotal = cart.Subtotal,
        ...
        Zephiel.Web.Services.OrderNotes.AddSystem(_db, order.Id,
    ...
    public async Task<IActionResult> PaymentCallback(string reference, string trxref)
    {
        ...
        if (order != null)
        {
            ...
            if (wasUnpaid)
            {
                ...
                _ = _whatsapp.NotifyOrderAsync(order.Id, Zephiel.Web.Services.WhatsAppOrderEvent.PaymentReceived);
            }
            ...
            if (outcome == Zephiel.Web.Services.FulfilOutcome.SoldOut)
            {
        ...
        try
        {
                ...
                && await _settings.GetBoolAsync("notifications.order_confirmed", true))
            {
                ...
                var items = order.Items.Select(i =>
                    new Zephiel.Web.Services.OrderEmailTemplate.Item(
                ...
                var introHtml = Zephiel.Web.Services.OrderEmailTemplate.ApplyPlaceholders(
                    intro, "#" + order.OrderNumber, order.CreatedAt, custName);
                var body = Zephiel.Web.Services.OrderEmailTemplate.Build(
                    heading: subject,
                    introHtml: introHtml,
                    orderNumber: order.OrderNumber,
            ...
            _ = _whatsapp.NotifyOrderAsync(order.Id, Zephiel.Web.Services.WhatsAppOrderEvent.OrderConfirmed);
    ...
    private async Task<string> BuildAdminOrderAlertAsync(Order order, string? customerEmail)
    {
        ...
        var items = order.Items.Select(i => new Zephiel.Web.Services.OrderEmailTemplate.Item(
            i.ProductName, i.VariantName, i.Quantity, i.LineTotal, AbsImg(i.ProductId))).ToList();
        ...
        return Zephiel.Web.Services.OrderEmailTemplate.Build(
    ...
    private async Task CaptureAbandonedCartAsync(string? email, CartViewModel cart)
    {
        ...
        if (existing == null)
        {
            _db.AbandonedCarts.Add(new Zephiel.Web.Models.Domain.AbandonedCart
            {
