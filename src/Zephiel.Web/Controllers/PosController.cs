...
public class PosController : Controller
{
    ...
    private readonly Zephiel.Web.Services.IStoreAccessService _access;
    ...
    private readonly Zephiel.Web.Services.ITransferWorkflowService _transfers;
    private readonly Zephiel.Web.Services.IManifestTokenService _manifestTokens;
    ...
    public PosController(ApplicationDbContext db, IStockService stock,
        ...
        Zephiel.Web.Services.IStoreAccessService access,
        ...
        Zephiel.Web.Services.ITransferWorkflowService transfers,
        Zephiel.Web.Services.IManifestTokenService manifestTokens)
    {
        _db = db;
        _stock = stock;
        _signIn = signIn;
    ...
    private async Task<bool> CanWorkAtStoreAsync(ApplicationUser user, int storeId)
    {
        ...
        if (roles.Any(r => Zephiel.Web.Areas.Admin.AdminSections.FullAccessRoles.Contains(r)))
            return true;
    ...
    {
        var basePath = "/" + Zephiel.Web.Infrastructure.StaffPaths.Pos;
        var manifest = new
        {
            name = "Zephiel POS",
            short_name = "SG POS",
    ...
    public async Task<IActionResult> TransferRequest([FromBody] PosTransferRequestDto req)
    {
        ...
        var (ok, error, id) = await _transfers.RequestAsync(new Zephiel.Web.Services.TransferRequest
        {
            FromStoreId = reg.StoreId,
            ToStoreId = req.ToStoreId,
            Note = reason,
            Items = req.Items.Where(i => i.Qty > 0)
                .Select(i => new Zephiel.Web.Services.TransferLine { ProductId = i.ProductId, Quantity = i.Qty }).ToList()
        }, userId);
    ...
    public async Task<IActionResult> TransferDispatch([FromBody] PosTransferIdDto req)
    {
        ...
        var items = t.Items.Select(i => new Zephiel.Web.Services.ItemQtyDto(i.Id, i.ApprovedQty ?? 0)).ToList();
    ...
    public async Task<IActionResult> TransferReceive([FromBody] PosReceiveDto req)
    {
        ...
        List<Zephiel.Web.Services.ReceiveLineDto> lines;
        var anyShort = false;
        if (req.Items != null && req.Items.Count > 0)
        {
            ...
            foreach (var l in req.Items)
            {
                ...
                lines.Add(new Zephiel.Web.Services.ReceiveLineDto(it.Id, recv, 0, 0)); // shortfall stays pending
            }
        }
        else
        {
            lines = t.Items.Select(i => new Zephiel.Web.Services.ReceiveLineDto(i.Id, i.PendingQty, 0, 0)).ToList();
        }
