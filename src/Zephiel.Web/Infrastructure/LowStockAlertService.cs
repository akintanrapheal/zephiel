...
public class LowStockAlertService : BackgroundService
{
    ...
    private async Task SweepAsync(CancellationToken ct)
    {
        ...
        if (sent)
        {
            ...
            _logger.LogInformation("Stock digest sent to {Email} ({Count} item(s), {Branches} branch(es)).", Zephiel.Web.Infrastructure.LogRedact.Email(adminEmail), items.Count, branchCount);
        }
        else
        {
