...
public class TransfersController : InventoryAreaController
{
    ...
    {
        if (!Zephiel.Web.Areas.Admin.AdminSections.IsFullAccess(User)) return Json(new { success = false, message = "Only an administrator can approve transfers." });
        var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
    ...
    {
        if (!Zephiel.Web.Areas.Admin.AdminSections.IsFullAccess(User)) return Json(new { success = false, message = "Only an administrator can reject transfers." });
        var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        var deny = await DenyIfNoStoreAccessAsync(id, from: true, to: false); if (deny != null) return deny;
