...
public class SettingsController : AdminBaseController
{
    ...
    private readonly Zephiel.Web.Services.IStorefrontCache _storefrontCache;
    private readonly IPermissionService _perms;

    public SettingsController(ISettingsService settings, ApplicationDbContext db,
        Zephiel.Web.Services.IStorefrontCache storefrontCache, IPermissionService perms)
    {
        _settings = settings;
        _db = db;
    ...
    public async Task<IActionResult> Save(string group, IFormCollection form)
    {
        ...
        var changed = updates
            ...
            .ToArray();
        var changes = Zephiel.Web.Services.AuditChanges.Build(changed);
